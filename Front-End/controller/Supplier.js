...
function getAllSuppliers() {
    $.ajax({
        url: "http://localhost:8082/api/v1/supplier/getAll",
        method: "GET",
        dataType: "json",
        headers: {
            "Authorization": "Bearer " + localStorage.getItem("token")
...
function SaveSupplier() {
    ...
    $.ajax({
        url: "http://localhost:8082/api/v1/supplier/save",
        method: "POST",
        data: jsonObj,
        ...
        success: function (resp, textStatus, jqxhr) {
            ...
            $("#btnSuppSave").prop("disabled", true);
            $("#btnSuppUpdate").prop("disabled", true);
            $("#btnSuppDelete").prop("disabled", true);
        },
...
function SupplierUpdate() {
    ...
    $.ajax({
        url: "http://localhost:8082/api/v1/supplier/update",
        method: "PATCH",
        data: jsonObj,
        contentType: "application/json",
        headers: {
...
$("#btnSuppClear").on("click", function () {
    $("#txtSupName").val("");
    $("#txtSupEmail").val("");
    $("#cmbSupCategory").val("");
    $("#txtSupAddLine01").val("");
...
$("#btnSuppDelete").on("click", function () {
    let code = $("#SupCode").val();
    if (code === ""){
...
function deleteSupplier(code) {
    $.ajax({
        url: "http://localhost:8082/api/v1/supplier/delete?"+"code="+code,
        method: "DELETE",
        dataType: "json",
        headers: {
            "Authorization": "Bearer " + localStorage.getItem("token")
...
$("#btnSupplierSearch").click(function () {
    ...
    if (searchType === "code"){
        supSearch(searchValue, "http://localhost:8082/api/v1/supplier/searchById?id=",searchType);
    } else if(searchType === "Name"){
        supSearch(searchValue,"http://localhost:8082/api/v1/supplier/searchByName?name=",searchType);
    }
})
...
$(document).on('click', '#SupplierTbl > tr', function(){
    ...
    $("#btnSuppSave").prop("disabled", true);
    $("#btnSuppUpdate").prop("disabled", false);
    $("#btnSuppDelete").prop("disabled", false);
